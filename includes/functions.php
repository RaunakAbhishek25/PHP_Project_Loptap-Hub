<?php
// includes/functions.php - Complete Functions File with INR Currency
...
function addReview($user_id, $laptop_id, $rating, $comment) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("INSERT INTO reviews (user_id, laptop_id, rating, comment) VALUES (?, ?, ?, ?)");
        $stmt->execute([$user_id, $laptop_id, $rating, $comment]);
        
        // Update laptop rating
        $stmt = $pdo->prepare("
            UPDATE laptops 
            SET rating = (SELECT AVG(rating) FROM reviews WHERE laptop_id = ?),
...
function getDashboardStats() {
    ...
    try {
        ...
        $stmt = $pdo->query("SELECT SUM(grand_total) as total FROM orders WHERE status != 'cancelled'");
